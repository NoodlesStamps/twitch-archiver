...
class Vod:
    ...
    def get_muted_segments(self):
        ...
        if _segments:
            _muted_segments = [
                MpegSegment(s["offset"], s["duration"], muted=True)
                for s in _segments["nodes"]
            ]
    ...
    def get_index_url(self, quality="best"):
        ...
        try:
            _r = self._api.get_request(
                f"https://usher.ttvnw.net/vod/{self.v_id}.m3u8", p=_p
            )
        ...
        # catch for sub-only VODs
            ...
            self._log.debug(
                "VOD %s is subscriber-only. Generating index URL.", self.v_id
            )
    ...
    def _get_playback_access_token(self):
        ...
        self._log.debug(
            "Token could not be retrieved for VOD %s: %s", self.v_id, _r.text
        )
        return ""
...
class ArchivedVod(Vod):
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, self.__class__):
            return (
                (self.s_id == other.s_id or self.v_id == other.v_id)
                and self.chat_archived == other.chat_archived
    ...
    def to_dict(self):
        ...
        return {
            ...
            "published_at": datetime.utcfromtimestamp(self.published_at).isoformat() if self.published_at else None,
            "thumbnail_url": self.thumbnail_url,
            "duration": self.duration,
            "chat_archived": self.chat_archived,
            "video_archived": self.video_archived,
        }
    ...
    def ordered_db_dict(self):
        ...
        return {
            ...
            "published_at": datetime.utcfromtimestamp(self.published_at),
            "thumbnail_url": self.thumbnail_url,
            "duration": self.duration,
            "muted_segments": str(self.get_muted_segments()),
            "chat_archived": self.chat_archived,
            "video_archived": self.video_archived,
        }
    ...
    def import_from_db(args: tuple):
        ...
        if "Z" in args[2]:
            _archived_vod.created_at = (
                datetime.strptime(args[2], "%Y-%m-%dT%H:%M:%SZ")
    ...
    @staticmethod
    def convert_from_vod(
        vod: Vod, chat_archived: bool = False, video_archived: bool = False
    ):
        """
